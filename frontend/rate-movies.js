function getMoviesToRate(user) {
  ...
  fetch("http://localhost:3000/movies")
    .then(res => res.json())
    .then(allMovies =>
      findUnratedMovies(current_user, allMovies, movieContainer)
    );
...
function showMovie(movie, user, context) {
  ...
  if (context != "frontPage-rating") {
    if (user.movies.includes(movie)) {
      console.log("Yes, this is one of our movies");
    }
  ...
  submitButton.addEventListener("click", event =>
    createInterest(movie, user, stars.value, hearts.value, context)
  );
...
function createInterest(movie, user, stars, hearts, context) {
  ...
  data = {
    movie_id: movie.id,
    user_id: parseInt(window.localStorage.current_user_id),
    star: stars,
    heart: hearts
  };
